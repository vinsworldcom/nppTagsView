...
bool COption::equal(const COption& opt) const
{
    ...
    if ( isStr() || isData() )
    {
        return _bufData.equal(opt._bufData);
    }
...
const void* COption::getData(int* psize ) const
{
    if ( !_bufData.empty() )
    {
        ...
        return (const void *) _bufData.data();
    }
    if ( psize )  *psize = 0;
    return NULL;
}
...
int COption::getData(void* pdata, int max_size) const
{
    if ( !_bufData.empty() )
    {
        ...
        return _bufData.copy( (byte_tt *) pdata, max_size );
    }
    if ( pdata && (max_size > 0) )  *((byte_tt *) pdata) = 0;
    return 0;
}
...
const TCHAR* COption::getStr(int* plen ) const
{
    if ( !_bufData.empty() )
    {
        ...
        return (const TCHAR *) _bufData.data();
    }
    if ( plen )  *plen = 0;
    return _T("\0");
}
...
int COption::getStr(TCHAR* pstr, int max_len) const
{
    if ( !_bufData.empty() )
    {
        ...
        max_len = _bufData.copy( (byte_tt *) pstr, max_len*sizeof(TCHAR) );
...
void COption::setData(const void* pdata, int nsize)
{
    ...
    if ( pdata && (nsize > 0) )
    {
        ...
        _bufData.assign( (const byte_tt*) pdata, nsize );
        while ( nn-- > 0 )
        {
            _bufData.append(0);
        }
...
void COption::setStr(const TCHAR* pstr, int nlen )
{
    ...
    if ( nlen > 0 )
    {
        ...
        _bufData.assign( (const byte_tt*) pstr, nlen*sizeof(TCHAR) );
        while ( nn-- )
        {
            _bufData.append(0);
        }
...
bool CIniOptionsReaderWriter::readDataOption(COptionsManager::opt_itr itr)
{
    ...
    unsigned int len = ::GetPrivateProfileString( itr->getOptionGroup(), 
        ...
        COptionsManager::MAX_STRSIZE - 1, _iniFilePathName.data() );
    if ( str[0] != _T('~') )
    {
        if ( len > 0 )
        {
...
bool CIniOptionsReaderWriter::readHexIntOption(COptionsManager::opt_itr itr)
{
    ...
    ::GetPrivateProfileString( itr->getOptionGroup(), 
        ...
        COptionsManager::MAX_NUMSTRSIZE - 3, _iniFilePathName.data() );
...
bool CIniOptionsReaderWriter::readIntOption(COptionsManager::opt_itr itr)
{
    unsigned int i = ::GetPrivateProfileInt( itr->getOptionGroup(), 
        itr->getOptionName(), itr->getInt(), _iniFilePathName.data() );
    itr->setUint(i);
...
bool CIniOptionsReaderWriter::writeDataOption(COptionsManager::const_opt_itr itr)
{
    ...
    if ( ::WritePrivateProfileString( itr->getOptionGroup(),
        itr->getOptionName(), str, _iniFilePathName.data() ) )
    {
        return true;
    }
...
{
    if ( ::WritePrivateProfileString(itr->getOptionGroup(),
        itr->getOptionName(), itr->getStr(), _iniFilePathName.data()) )
    {
        return true;
    }
